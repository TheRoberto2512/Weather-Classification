...
def ensemble_main(dataset, votazione="soft", pesata=True, show_results=True):
    '''
    Funzione per addestrare un classificatore composto.
    
    Parametri:
    - dataset: tupla contenente i dati di training e di test già splittati.
    - votazione: tipologia di votazione da utilizzare (default = soft).
    - pesata: se True, i contributi dei classificatori sono pesati in base all'accuratezza (default = True).
    ...
    accuracy_KNN, pred_KNN = custom_KNN_main(dataset = dataset, votazione = votazione, show_results=False)
    accuracy_NaiveBayes, pred_NaiveBayes = naive_bayes_main(dataset = dataset, votazione = votazione, show_results=False)
    accuracy_Decision_Tree, pred_Decision_Tree = decision_tree_main(dataset = dataset, votazione = votazione, show_results=False)
    accuracy_SVM, pred_SVM = SVM_main(dataset = dataset, votazione = votazione, show_results=False)
...
def tuning_iperparametri(dataset):
    ...
    acc_matrix = np.zeros((len(contributo_pesato), len(tipi_votazione)))
    ...
    for j, tipo in enumerate(tipi_votazione):
        for i, contributo in enumerate(contributo_pesato):
            ...
            acc_matrix[i, j] = ensemble_main(dataset, votazione = tipo, pesata = contributo, show_results=False)
            # salva l'accuratezza del classificatore composto per ogni combinazione di iperparametri
            
            print("Tipo di votazione: \"%s\", contributo pesato: %s - Accuratezza: %.5f" % (tipo, contributo, acc_matrix[i,j]))
        
        print("\n")
    
    # indice della combinazione di iperparametri con l'accuratezza più alta
    i, j = np.unravel_index(np.argmax(acc_matrix, axis=None), acc_matrix.shape)
    print(f"Miglior {Colors.GREEN}Accuratezza{Colors.RESET}: %.5f (Usando tipo di votazione \"%s\" e con contributo pesato = %s)" % (acc_matrix[i,j], tipi_votazione[j], contributo_pesato[i]) )
